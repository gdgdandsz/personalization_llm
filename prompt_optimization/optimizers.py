...
class PromptOptimizer(ABC):
    ...
    @abstractmethod
    def expand_candidates(self, prompts, task, llama, train_exs):
        pass

class ProTeGi(PromptOptimizer):
    """ ProTeGi: Prompt Optimization with Textual Gradients
    """
    def _sample_error_str(self, texts, labels, preds, task, n=20):
        ...
        sample_texts = [texts[i].values[0] for i in sample_idxs]
    ...
    def summarize_text(self, text):
        ...
        # 转换为NumPy数组并且确保所有后续操作都使用这个数组
        embeddings_np = embeddings.cpu().numpy()  # 将Tensor从CUDA移动到CPU并转换为NumPy数组

        num_clusters = min(len(sentences), 5)
        clustering_model = KMeans(n_clusters=num_clusters)
        clustering_model.fit(embeddings_np)  # 使用NumPy数组进行聚类
        cluster_assignment = clustering_model.labels_

        summarized_text = []
        for i in range(num_clusters):
            cluster_index = np.where(cluster_assignment == i)[0]
            # 使用NumPy数组进行所有计算
            central_sentence_idx = cluster_index[np.argmin(np.sum(np.square(embeddings_np[cluster_index] - clustering_model.cluster_centers_[i]), axis=1))]
            summarized_text.append(sentences[central_sentence_idx])
    ...
    def apply_gradient(self, prompt, error_str, feedback_str, steps_per_gradient, n=1):
        """ Incorporate feedback gradient into a prompt."""
        # 使用 summarize_text 方法优化 error_str 和 feedback_str
        error_summary = self.summarize_text(error_str)
        feedback_summary = self.summarize_text(feedback_str)
        ...
        But it gets the following issues:
        {error_summary}

        Based on these issues, the problem with this prompt is that:
        {feedback_summary}

        I need to write {steps_per_gradient} different improved prompts.
        Each prompt is wrapped with <START> and <END>.
        """
        transformation_prompt = '\n'.join([line.lstrip() for line in transformation_prompt.split('\n')])
        res = utils.chatgpt(transformation_prompt, n=n)
    ...
    def generate_synonyms(self, prompt_section, n=3):
        ...
        rewriter_prompt = f"Generate a variation of the following instruction while keeping the semantic meaning. Do not change the output format.\n\nInput: {prompt_section}\n\nOutput:"
        new_instructions = utils.chatgpt(rewriter_prompt, n=n)
        new_instructions = [x for x in new_instructions if x]
        return new_instructions

    def get_gradients(self, prompt, task_section, task, llama, texts, labels, preds):
    ...
    def expand_candidates(self, prompts, task, llama, train_exs, label_score):
        ...
        for prompt in tqdm(prompts, desc=f'expanding {len(prompts)} prompts'):
            ...
            _, texts, labels, preds = task.evaluate(llama, prompt, minibatch, label_score)

            # get gradients
            new_task_sections = []
            if self.opt['n_gradients'] > 0:
                gradients = self.get_gradients(prompt, task_section, task, llama, texts, labels, preds)
                new_task_sections = []
            ...
            if len(new_sections) > self.opt['max_expansion_factor']:
                if self.opt['reject_on_errors']:
                    ...
                    error_scores = self.bf_eval(tmp_new_prompts, error_exs, task, llama, self.scorer, max_threads=self.max_threads)
    ...
    def score_candidates(self, prompts, task, llama, train_exs, label_score):
        """ Score a list of prompts."""
        if len(prompts) == 1:
            return [1.0]

        evals = self.evaluator_fn(
            prompts, train_exs, task, llama, label_score,
            scorer=self.scorer,
